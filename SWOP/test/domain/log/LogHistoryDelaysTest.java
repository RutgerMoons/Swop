...
public class LogHistoryDelaysTest {
	...
	public void shiftTest1(){
		Delay delay = new Delay(new ImmutableClock(1,300), new ImmutableClock(1,400));
		for(int i = 0; i < 4;i++){
	...
	public void shiftTest2(){
		...
		Delay delay1 = new Delay(new ImmutableClock(1,400), new ImmutableClock(1,550));
		for(int i = 0; i < 5;i++){
			history.addNewDelay(delay1);
		}
		Delay delay3 = new Delay(new ImmutableClock(2,300), new ImmutableClock(2,400));
		for(int i = 0; i < 3;i++){
	...
	public void addNewDelayNoDelayTest(){
		Delay delay = new Delay(new ImmutableClock(1,300), new ImmutableClock(1,300));
		history.addNewDelay(delay);
	}
}
